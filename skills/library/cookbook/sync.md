...
- For `skill:`, `agent:`, `prompt:` dependencies:
...
- For `mcp:` dependencies:
  - Look up the MCP entry in `library.mcps`
  - Check if the MCP is configured in `.mcp.json` (project) or `~/.claude/settings.json` (global)
  - If not configured, install it per the use.md Step 3a MCP resolution steps
  - If already configured but the config differs from the library definition (e.g., updated args or url), update the config to match
  - Note any MCP configs that were added or updated in the report
