...
- `mcp:name` — detect MCP server usage by looking for:
  - `browser_navigate`, `browser_snapshot`, `browser_click`, or `mcp__playwright__` tool references → `mcp:playwright`
  - References to "Playwright MCP" or "using-playwright-mcp" → `mcp:playwright`
  - `send_message` via Beeper, `beeper-send.sh`, `beeper-write.sh`, or references to "Beeper MCP" or "using-beeper" → `mcp:beeper`
  - Any other `mcp__<server>__` tool prefixes → ask the user which MCP server it needs
- Verify each dependency already exists in `library.yaml` (skills/agents/prompts) or `library.mcps` (MCPs). Warn the user if not found.
  - If skill/agent/prompt deps don't exist, add them to `library.yaml` first. If those files have dependencies, add them recursively.
