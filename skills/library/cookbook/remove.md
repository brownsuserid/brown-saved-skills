...
- If this skill has `mcp:` dependencies, check if any OTHER installed skills also depend on the same MCP server:
  - If this is the **last** installed skill using that MCP: ask the user "No other installed skills use the `<name>` MCP server. Remove its config from `<.mcp.json / settings.json>`?"
  - If the user confirms, remove the MCP key from the config file
  - If other installed skills still use the MCP: leave the MCP config in place
