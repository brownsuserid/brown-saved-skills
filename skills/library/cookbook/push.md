...
- Copy the local version into the clone:
  ...
  ```
- Create a branch, stage ONLY the relevant changes, commit, and open a PR:
  ```bash
  cd "$tmp_dir"
  git checkout -b library/<name>
  git add <skill_path_in_repo>
  git commit -m "library: updated <name> <brief description of what changed>"
  git push -u origin library/<name>
  gh pr create --title "library: updated <name>" --body "<brief description of what changed>"
  ```
- Clean up:
  ```bash
  rm -rf "$tmp_dir"
