...
### 2. Read the Catalog
- Read `library.yaml`
- Parse all entries from `library.skills`, `library.agents`, `library.prompts`, and `library.mcps`
...
|------|-------------|--------|
| playwright | Browser automation via Playwright | configured (.mcp.json) |
| beeper | Unified messaging via Beeper Desktop | not configured |

Check MCP status by:
- Reading `.mcp.json` in the current directory for project-level config
- Reading `~/.claude/settings.json` `mcpServers` for global-level config
- If the MCP name appears as a key in either: "configured (`<location>`)"
