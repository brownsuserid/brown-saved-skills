...
- **LIBRARY_REPO_URL**: `https://github.com/Brain-Bridge-AI/the-library.git`
...
- `mcp:name` — references an MCP server in the library catalog (`library.mcps`)

When resolving dependencies:
- For `skill:`, `agent:`, `prompt:` refs: look up in `library.yaml`, fetch from source (recursively)
- For `mcp:` refs: look up in `library.mcps`, write MCP server config to `.mcp.json` (project scope) or `~/.claude/settings.json` mcpServers (global scope), matching the same scope the user chose for the skill install
...
library:
  mcps:
    ...
    - name: beeper
      description: Unified messaging via Beeper Desktop
      type: http
      url: "http://localhost:23373/v0/mcp"
      note: Requires Beeper Desktop running with API enabled
  ...
  skills:
    ...
    - name: browser-scraper
      description: Scrape web pages using Playwright browser automation
      source: https://github.com/myorg/private-skills/blob/main/skills/browser-scraper/SKILL.md
